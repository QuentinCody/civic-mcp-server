...
var SchemaInferenceEngine = class {
  ...
  static findCollections(obj, path, schemas) {
  ...
  static handleSingleEntity(data, schemas) {
  ...
  static createSchemaFromArray(array, tableName) {
    const columnTypes = {};
    const sampleData = [];
  ...
  static createSchemaFromObject(obj, tableName) {
    const columnTypes = {};
  ...
  static extractFields(obj, columnTypes) {
    const rowData = {};
  ...
  static getTableName(path) {
    if (path.length === 0) return "main_table";
    const lastComponent = path[path.length - 1];
    return lastComponent === "edges" && path.length > 1 ? this.sanitizeTableName(path[path.length - 2]) : this.sanitizeTableName(lastComponent);
  }
  static sanitizeTableName(name) {
...
var DataInsertionEngine = class {
  ...
  static async insertData(data, schemas, sql) {
  ...
  static async findAndInsertCollections(obj, path, schemas, sql, foundCallback) {
    if (!obj || typeof obj !== "object") return;
    if (Array.isArray(obj) && obj.length > 0) {
  ...
  static async insertSingleEntity(data, schemas, sql) {
  ...
  static async insertArray(array, tableName, schema, sql) {
    for (const item of array) {
      await this.insertSingleRow(item, tableName, schema, sql);
    }
  }
  static async insertSingleRow(obj, tableName, schema, sql) {
  ...
  static findOriginalKey(obj, sanitizedKey) {
    const keys = Object.keys(obj);
    if (keys.includes(sanitizedKey)) return sanitizedKey;
    return keys.find(
      (key) => SchemaInferenceEngine["sanitizeColumnName"](key) === sanitizedKey
    ) || null;
  }
};
...
var JsonToSqlDO = class extends DurableObject3 {
  ...
  async createTables(schemas) {
    for (const [tableName, schema] of Object.entries(schemas)) {
      const columnDefs = Object.entries(schema.columns).map(([name, type]) => `${name} ${type}`).join(", ");
      ...
      async ({ query, variables }) => {
        try {
          ...
          if (graphqlResult.errors && !graphqlResult.data) {
            return { content: [{ type: "text", text: JSON.stringify(graphqlResult, null, 2) }] };
          }
...
// .wrangler/tmp/bundle-q6vpvi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-q6vpvi/middleware-loader.entry.ts
