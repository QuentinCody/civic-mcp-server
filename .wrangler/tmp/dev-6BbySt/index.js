...
var JsonToSqlDO = class {
  // Use any type for DO's SQLite storage
  ...
  async insertArrayIntoTable(schema, data) {
    ...
    for (const record of data) {
      if (record && typeof record === "object") {
        ...
        const insertSql = `INSERT INTO ${schema.tableName} (${columnNames.join(", ")}) VALUES (${columnNames.map(() => "?").join(", ")})`;
  ...
  async generateLlmMetadata() {
    try {
      const tablesResult = this.db.exec("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%'");
      const tables = [];
      const tableRows = Array.isArray(tablesResult) ? tablesResult : tablesResult.toArray();
      let totalRecords = 0;
      for (const tableRow of tableRows) {
        const tableName = tableRow.name;
        const schemaResult = this.db.exec(`PRAGMA table_info(${tableName})`);
        const schemaRows = Array.isArray(schemaResult) ? schemaResult : schemaResult.toArray();
        const countResult = this.db.exec(`SELECT COUNT(*) as count FROM ${tableName}`);
        const countRows = Array.isArray(countResult) ? countResult : countResult.toArray();
        const recordCount = countRows.length > 0 ? countRows[0].count : 0;
  ...
  async executeSql(sqlQuery, params) {
    ...
    try {
      const result = params && params.length > 0 ? this.db.exec(sqlQuery, ...params) : this.db.exec(sqlQuery);
      const rows = Array.isArray(result) ? result : result.toArray();
      return {
        success: true,
        results: rows,
        rowCount: rows.length,
        columns: result.columnNames || Object.keys(rows[0] || {}),
...
// .wrangler/tmp/bundle-f0ZgnJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-f0ZgnJ/middleware-loader.entry.ts
