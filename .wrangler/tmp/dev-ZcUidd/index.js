...
// src/do.ts
var JsonToSqlDO = class {
  // The SQL API from DO state
  constructor(state, env2) {
    this.state = state;
    this.env = env2;
    console.log("JsonToSqlDO instance created");
    this.sql = state.storage.sql;
    console.log("SQLite API initialized:", typeof this.sql);
  }
  static {
    __name(this, "JsonToSqlDO");
  }
  // Enhanced method: Process JSON data, infer schema, and create SQLite tables
  async processAndStoreJson(jsonData) {
    console.log("processAndStoreJson called with data:", JSON.stringify(jsonData).slice(0, 200) + "...");
    try {
      const inferredSchemas = this.inferSchemaFromJSON(jsonData);
      console.log(`Inferred ${inferredSchemas.length} table schema(s)`);
      await this.createTablesFromSchemas(inferredSchemas);
      console.log("SQLite tables created successfully");
      await this.insertDataIntoTables(inferredSchemas, jsonData);
      console.log("Data inserted into SQLite tables");
      const llmMetadata = await this.generateLlmMetadata();
      return {
        message: "JSON processed and stored in SQLite",
        dataId: this.state.id.toString(),
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
        ...llmMetadata
      };
    } catch (error3) {
      console.error("Error processing JSON:", error3);
      return {
  ...
  // Create SQLite tables from inferred schemas
  async createTablesFromSchemas(inferredSchemas) {
    for (const schema of inferredSchemas) {
      const createTableSql = this.generateCreateTableSql(schema);
      console.log(`Creating table ${schema.tableName}: ${createTableSql}`);
      try {
        await this.sql.exec(createTableSql);
        console.log(`Successfully created table: ${schema.tableName}`);
      } catch (error3) {
        console.error(`Failed to create table ${schema.tableName}:`, error3);
        throw error3;
      }
    }
  }
  ...
  generateCreateTableSql(schema) {
    if (schema.columns.length === 0) {
      return `CREATE TABLE IF NOT EXISTS ${schema.tableName} (
				...
				_empty_table TEXT DEFAULT 'No columns inferred'
			)`;
    }
    const columnDefinitions = schema.columns.map(
  ...
  // Insert actual data into the created tables
  async insertDataIntoTables(inferredSchemas, jsonData) {
    const dataArrays = this.extractDataArrays(jsonData);
    for (const schema of inferredSchemas) {
      const tableData = dataArrays.get(schema.originalName || schema.tableName);
      if (tableData && tableData.length > 0) {
        await this.insertArrayIntoTable(schema, tableData);
      }
    }
  }
  // Extract data arrays from JSON for insertion
  extractDataArrays(jsonData) {
    const dataArrays = /* @__PURE__ */ new Map();
    if (jsonData && typeof jsonData === "object") {
      if (jsonData.data && typeof jsonData.data === "object") {
        this.extractArraysFromLevel(jsonData.data, dataArrays, "root");
      } else {
        this.extractArraysFromLevel(jsonData, dataArrays, "root");
      }
      if (jsonData.errors && Array.isArray(jsonData.errors)) {
        dataArrays.set("errors", jsonData.errors);
      }
    }
    return dataArrays;
  }
  // Extract arrays from a data level for insertion
  extractArraysFromLevel(data, dataArrays, parentName) {
    if (Array.isArray(data)) {
      dataArrays.set(parentName, data);
    } else if (data && typeof data === "object") {
      for (const [key, value] of Object.entries(data)) {
        if (Array.isArray(value)) {
          dataArrays.set(key, value);
        } else if (value && typeof value === "object") {
          if (this.isGraphQLConnection(value)) {
            const nodes = this.extractNodesFromConnection(value);
            if (nodes.length > 0) {
              dataArrays.set(key, nodes);
            }
          } else {
            this.extractArraysFromLevel(value, dataArrays, key);
          }
        }
      }
    }
  }
  // Insert array data into a specific table
  async insertArrayIntoTable(schema, data) {
    if (data.length === 0 || schema.columns.length === 0) {
      return;
    }
    console.log(`Inserting ${data.length} records into ${schema.tableName}`);
    const columnNames = schema.columns.map((col) => col.name);
    const placeholders = columnNames.map(() => "?").join(", ");
    const insertSql = `INSERT INTO ${schema.tableName} (${columnNames.join(", ")}) VALUES (${placeholders})`;
    try {
      const stmt = await this.sql.prepare(insertSql);
      for (const record of data) {
        if (record && typeof record === "object") {
          const values = columnNames.map((colName) => {
            const originalColName = this.getOriginalColumnName(colName);
            let value = record[originalColName];
      ...
      console.log(`Successfully inserted ${data.length} records into ${schema.tableName}`);
    } catch (error3) {
      console.error(`Failed to insert data into ${schema.tableName}:`, error3);
      throw error3;
    }
  }
  // Get original column name (reverse the sanitization for data lookup)
  getOriginalColumnName(sanitizedName) {
    if (sanitizedName.startsWith("col_")) {
      return sanitizedName.substring(4);
    }
    return sanitizedName.replace(/_/g, "");
  }
  // Helper method: Infer SQLite schema from JSON data
  inferSchemaFromJSON(jsonData) {
    const schemas = [];
    if (jsonData && typeof jsonData === "object") {
      if (jsonData.data && typeof jsonData.data === "object") {
        this.processDataLevel(jsonData.data, schemas, "root");
      } else {
        this.processDataLevel(jsonData, schemas, "root");
      }
      if (jsonData.errors && Array.isArray(jsonData.errors)) {
        this.processArrayAsTable(jsonData.errors, schemas, "errors");
      }
    }
    return schemas;
  }
  // Process a data level (object or array) and extract table schemas
  processDataLevel(data, schemas, parentName) {
    if (Array.isArray(data)) {
      this.processArrayAsTable(data, schemas, parentName);
    } else if (data && typeof data === "object") {
      for (const [key, value] of Object.entries(data)) {
        if (Array.isArray(value)) {
          this.processArrayAsTable(value, schemas, key);
        } else if (value && typeof value === "object") {
          if (this.isGraphQLConnection(value)) {
            const nodes = this.extractNodesFromConnection(value);
            if (nodes.length > 0) {
              this.processArrayAsTable(nodes, schemas, key);
            }
          } else {
            this.processDataLevel(value, schemas, key);
          }
        }
      }
    }
  }
  // Check if an object follows GraphQL connection pattern (has edges/nodes)
  isGraphQLConnection(obj) {
    return obj && typeof obj === "object" && obj.edges && Array.isArray(obj.edges);
  }
  // Extract nodes from GraphQL connection structure
  extractNodesFromConnection(connection) {
    const nodes = [];
  ...
  // Process an array as a database table
  processArrayAsTable(array, schemas, tableName) {
    if (array.length === 0) {
      schemas.push({
    ...
    for (const item of array) {
      if (item && typeof item === "object") {
        for (const [columnName, value] of Object.entries(item)) {
          const inferredType = this.inferColumnType(value);
          const existingType = columnTypes.get(columnName);
          if (!existingType) {
            columnTypes.set(columnName, inferredType);
          } else if (existingType !== inferredType) {
            columnTypes.set(columnName, "TEXT");
          }
        }
      }
    }
    const columns = Array.from(columnTypes.entries()).map(([name, type]) => ({
  ...
  // Generate LLM-friendly metadata from the actual database
  async generateLlmMetadata() {
    try {
      const tablesResult = await this.sql.prepare("SELECT name FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%'").all();
      const tables = [];
      let totalRecords = 0;
      for (const tableRow of tablesResult) {
        const tableName = tableRow.name;
        const schemaResult = await this.sql.prepare(`PRAGMA table_info(${tableName})`).all();
        const countResult = await this.sql.prepare(`SELECT COUNT(*) as count FROM ${tableName}`).first();
        ...
        tables.push({
          ...
          sample_query: `SELECT * FROM ${tableName} LIMIT 5;`
        });
      }
      return {
        tables,
        ...
        schema_summary: `Database contains ${tables.length} table(s) with ${totalRecords} total records. Available tables: ${tables.map((t) => t.table_name).join(", ")}`
      };
    } catch (error3) {
      console.error("Error generating LLM metadata:", error3);
      return {
        tables: [],
        table_count: 0,
        total_records: 0,
        schema_summary: "Error reading database schema",
        error: error3 instanceof Error ? error3.message : String(error3)
      };
    }
  }
  // Execute SQL queries against the stored data
  async executeSql(sqlQuery, params) {
    ...
    try {
      let result;
      if (params && params.length > 0) {
        const stmt = await this.sql.prepare(sqlQuery);
      ...
      console.error(`SQL execution error: ${error3 instanceof Error ? error3.message : String(error3)}`);
      return {
        success: false,
        error: error3 instanceof Error ? error3.message : String(error3),
        executedAt: (/* @__PURE__ */ new Date()).toISOString()
  ...
  // Fetch handler to route requests to appropriate methods
  async fetch(request) {
    const url = new URL(request.url);
    const path = url.pathname;
    try {
      if (path === "/processAndStoreJson" && request.method === "POST") {
        const requestData = await request.json();
        const result = await this.processAndStoreJson(requestData.jsonData);
        return new Response(JSON.stringify(result), {
          headers: { "Content-Type": "application/json" }
        });
      }
      if (path === "/executeSql" && request.method === "POST") {
        const requestData = await request.json();
        const result = await this.executeSql(requestData.sqlQuery, requestData.params);
        return new Response(JSON.stringify(result), {
          headers: { "Content-Type": "application/json" }
        });
      }
      return new Response(`JsonToSqlDO: Path ${path} not found. Available: /processAndStoreJson (POST), /executeSql (POST)`, {
        status: 404,
        headers: { "Content-Type": "text/plain" }
      });
    } catch (error3) {
      console.error("JsonToSqlDO fetch error:", error3);
      const errorMessage = error3 instanceof Error ? error3.message : String(error3);
      return new Response(JSON.stringify({
        error: "JsonToSqlDO processing error",
        message: errorMessage,
        timestamp: (/* @__PURE__ */ new Date()).toISOString()
      }), {
        status: 500,
...
function setGlobalEnvironment(env2) {
  currentEnvironment = env2;
  console.error("Global environment set for tools");
}
__name(setGlobalEnvironment, "setGlobalEnvironment");
function getGlobalEnvironment() {
  return currentEnvironment;
}
__name(getGlobalEnvironment, "getGlobalEnvironment");
var CivicMCP = class extends McpAgent {
  constructor() {
    super(...arguments);
    ...
    this.CIVIC_GRAPHQL_ENDPOINT = "https://civicdb.org/api/graphql";
    this.CIVIC_API_VERSION_HEADER = "application/vnd.civicdb.v2+json";
  }
  static {
    __name(this, "CivicMCP");
  }
  // Required for CIViC API V2
  async init() {
    console.error("CivicMCP Server initializing...");
    this.server.tool(
      "civic_graphql_query",
      // Tool description (max 1024 chars for this string)
      `Executes GraphQL queries against CIViC API (V2: ${this.CIVIC_GRAPHQL_ENDPOINT}) for Clinical Interpretations of Variants in Cancer, processes the JSON response into structured SQLite tables, and returns metadata for subsequent SQL querying. This tool does NOT return the raw JSON data directly. Instead, it provides a data_access_id that can be used with other tools to query the staged data. Query genes, variants, evidence, assertions, etc. Example (gene V2): '{ gene(id: 12) { id name evidenceItems { totalCount } } }'. Example (variants V2 w/ pagination): '{ variants(first: 5) { edges { node { id name } } pageInfo { endCursor hasNextPage } } }'. IMPORTANT: Always use V2! Before ANY query, ALWAYS run introspection to confirm target fields/ops exist in the V2 schema. Use small page counts initially. Returns: data_access_id and processing_details for SQL querying.`,
      {
        // Input schema
      ...
      async ({ query, variables }) => {
        ...
        const resultFromGraphQL = await this.executeCivicGraphQLQuery(query, variables);
        if (resultFromGraphQL.errors && !resultFromGraphQL.data) {
          return {
            content: [{
              type: "text",
              text: JSON.stringify(resultFromGraphQL, null, 2)
            }]
          };
        }
        try {
          const env2 = getGlobalEnvironment();
          if (!env2 || !env2.JSON_TO_SQL_DO) {
            console.error("Environment or JSON_TO_SQL_DO binding not available");
            return {
              content: [{
                type: "text",
                text: JSON.stringify({
                  error: "Environment configuration error",
                  message: "JSON_TO_SQL_DO binding not available. Make sure the Durable Object is properly configured in wrangler.jsonc",
                  original_graphql_result: resultFromGraphQL
                }, null, 2)
              }]
            };
          }
          const userAccessId = crypto.randomUUID();
          const doId = env2.JSON_TO_SQL_DO.idFromName(userAccessId);
          const stub = env2.JSON_TO_SQL_DO.get(doId);
          console.error(`Generated user access ID: ${userAccessId}`);
          console.error(`Created DO with ID: ${doId.toString()}`);
          console.error("Calling DO processAndStoreJson...");
          const doResponse = await stub.fetch(new Request(`https://do-internal/processAndStoreJson`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ jsonData: resultFromGraphQL })
          }));
          if (!doResponse.ok) {
            const errorText = await doResponse.text();
            console.error(`DO processAndStoreJson failed: ${doResponse.status} - ${errorText}`);
            return {
              content: [{
                type: "text",
                text: JSON.stringify({
                  error: "Data staging failed",
          ...
          const processingMetadata = await doResponse.json();
          console.error(`DO processing completed: ${JSON.stringify(processingMetadata).slice(0, 200)}...`);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
                data_access_id: userAccessId,
                processing_details: processingMetadata
              }, null, 2)
            }]
          };
        } catch (doError) {
          console.error(`DO staging error: ${doError instanceof Error ? doError.message : String(doError)}`);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
                error: "Data staging error",
                message: doError instanceof Error ? doError.message : String(doError),
                original_graphql_result: resultFromGraphQL
              }, null, 2)
            }]
          };
    ...
    this.server.tool(
      "civic_query_sql",
      `Execute read-only SQL queries against spilled CIViC data stored in SQLite. Use the data_access_id returned by civic_graphql_query to query the staged data. Supports standard SQL SELECT operations with WHERE clauses, JOINs, aggregations, etc. Returns query results as JSON. Example usage: After running civic_graphql_query and receiving a data_access_id, you can query the staged tables like 'SELECT * FROM types WHERE kind = "OBJECT" LIMIT 10'. Always reference the table schema information returned by civic_graphql_query to understand available tables and columns.`,
      {
        // Input schema
        data_access_id: z.string().describe(
          "The data access ID returned by civic_graphql_query. This identifies which staged dataset to query."
        ),
        sql: z.string().describe(
          "SQL query to execute against spilled data. Must be a SELECT statement. Use table and column names from the schema returned by civic_graphql_query."
        ),
        params: z.array(z.string()).optional().describe(
          "Optional parameters for the SQL query (for prepared statements with ? placeholders)."
        )
      },
      // Tool execution function
      async ({ data_access_id, sql, params }) => {
        console.error(`Executing civic_query_sql for data_access_id: ${data_access_id}`);
        console.error(`SQL query: ${sql.slice(0, 200)}...`);
        try {
          const env2 = getGlobalEnvironment();
          if (!env2 || !env2.JSON_TO_SQL_DO) {
            console.error("Environment or JSON_TO_SQL_DO binding not available");
          ...
          try {
            const doId = env2.JSON_TO_SQL_DO.idFromName(data_access_id);
            const stub = env2.JSON_TO_SQL_DO.get(doId);
            console.error(`Calling DO executeSql for DO ID: ${doId.toString()}`);
            const doResponse = await stub.fetch(new Request(`https://do-internal/executeSql`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                sqlQuery: sql,
            ...
            if (!doResponse.ok) {
              const errorText = await doResponse.text();
              console.error(`DO executeSql failed: ${doResponse.status} - ${errorText}`);
              return {
                content: [{
                  type: "text",
                  text: JSON.stringify({
            ...
            const queryResult = await doResponse.json();
            console.error(`DO SQL execution completed: ${JSON.stringify(queryResult).slice(0, 200)}...`);
            return {
              content: [{
                type: "text",
            ...
            console.error(`Invalid data_access_id or DO access error: ${doIdError instanceof Error ? doIdError.message : String(doIdError)}`);
            return {
              content: [{
                type: "text",
                text: JSON.stringify({
                  success: false,
                  error: "Invalid data access ID",
                  message: `Unable to access data with ID: ${data_access_id}. The data may have expired or the ID is invalid.`,
                  details: doIdError instanceof Error ? doIdError.message : String(doIdError)
                }, null, 2)
              }]
            };
          }
        } catch (error3) {
          console.error(`SQL querying error: ${error3 instanceof Error ? error3.message : String(error3)}`);
          return {
            content: [{
              type: "text",
              text: JSON.stringify({
                success: false,
                error: "SQL querying error",
    ...
    console.error("CivicMCP Server initialized and tools registered (civic_graphql_query, civic_query_sql).");
  }
  // Helper function to execute CIViC GraphQL queries (V2)
  async executeCivicGraphQLQuery(query, variables) {
    try {
      const headers = {
        "Content-Type": "application/json",
        "Accept": this.CIVIC_API_VERSION_HEADER,
        // Crucial for CIViC API V2
      ...
      console.error(`Client-side error during CIViC GraphQL request: ${error3 instanceof Error ? error3.message : String(error3)}`);
...
var src_default = {
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    setGlobalEnvironment(env2);
    if (url.pathname === "/sse" || url.pathname.startsWith("/sse/")) {
      return CivicMCP.serveSSE("/sse").fetch(request, env2, ctx);
    }
...
// .wrangler/tmp/bundle-RJ4bgl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RJ4bgl/middleware-loader.entry.ts
