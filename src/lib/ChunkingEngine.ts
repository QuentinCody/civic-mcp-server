...
export class ChunkingEngine {
	...
	private generateChunkingRules(): void {
		...
		this.chunkingRules = [
			...
			// CIViC-specific large fields
			{ fieldName: 'clinicalTrials', typeName: '*', chunkThreshold: 4096, priority: 'size-based', reason: 'Clinical trial data can be extensive' },
			{ fieldName: 'sourceMetadata', typeName: '*', chunkThreshold: 2048, priority: 'size-based', reason: 'Source metadata can contain large nested objects' },
	...
	private isLikelyLargeContent(fieldInfo: GraphQLFieldInfo): boolean {
		const largeContentIndicators = [
			...
			'metadata', 'details', 'comments', 'notes', 'evidence', 'clinical'
		];
